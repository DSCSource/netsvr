...
fill_prefix(char* ptr) {
	uint32_t handle = 0;//--netsvr_current_handle();
...
mallctl_int64(const char* name, size_t* newval) {
	...
	// netsvr_error(NULL, "name: %s, value: %zd\n", name, v);
	return v;
}
...
mallctl_opt(const char* name, int* newval) {
	...
	if(newval) {
		...
		if(ret == 0) {
			//netsvr_error(NULL, "set new value(%d) for (%s) succeed\n", *newval, name);
		} else {
			//netsvr_error(NULL, "set new value(%d) for (%s) failed: error -> %d\n", *newval, name, ret);
		}
	} else {
		je_mallctl(name, &v, &len, NULL, 0);
	}
...
void *
netsvr_realloc(void *ptr, size_t size) {
	if (ptr == NULL) return netsvr_malloc(size);
...
void *
netsvr_calloc(size_t nmemb,size_t size) {
...
void *
netsvr_memalign(size_t alignment, size_t size) {
...
void 
memory_info_dump(void) {
  //netsvr_error(NULL, "No jemalloc");
}

size_t 
mallctl_int64(const char* name, size_t* newval) {
	//netsvr_error(NULL, "No jemalloc : mallctl_int64 %s.", name);
	return 0;
}

int 
mallctl_opt(const char* name, int* newval) {
//netsvr_error(NULL, "No jemalloc : mallctl_opt %s.", name);
	return 0;
}
...
dump_c_mem() {
	int i;
	size_t total = 0;
	//netsvr_error(NULL, "dump all service mem:");
	for(i=0; i<SLOT_SIZE; i++) {
		struct mem_data* data = &mem_stats[i];
		if(data->handle != 0 && data->allocated != 0) {
			total += data->allocated;
			//netsvr_error(NULL, "0x%x -> %zdkb", data->handle, data->allocated >> 10);
		}
	}
	//netsvr_error(NULL, "+total: %zdkb",total >> 10);
}

char *
netsvr_strdup(const char *str) {
	size_t sz = strlen(str);
	char * ret = netsvr_malloc(sz+1);
	memcpy(ret, str, sz+1);
	return ret;
}
...
void
netsvr_debug_memory(const char *info) {
	// for debug use
	uint32_t handle = 0;//--netsvr_current_handle();
